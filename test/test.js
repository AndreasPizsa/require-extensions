...
describe('require-extension', function(){
    ...
    describe('Use with glob-fs', function(){
      it('finds files with require.extensions.glob', function(){
        require.extensions['.coffee'] = function(){}
        const glob = require('glob-fs')();
        const files = glob.readdirSync('*'+require.extensions.glob, { cwd: 'test/fixtures/'});
        function equal(to) {
          return function(item) { return item === to; }
        }
        assert.ok(files.find(equal('a.json')));
        assert.ok(files.find(equal('a.node')));
        assert.ok(files.find(equal('a.js')));
        assert.ok(files.find(equal('a.coffee')));
      })
    })
});
